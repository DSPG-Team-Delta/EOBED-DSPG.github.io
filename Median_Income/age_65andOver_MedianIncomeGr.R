...
plot65Inc <- function(county_merge, tract_merge, groups = NULL, pal, myDomain, cityLng,
   ...
   miMap<-leaflet() %>%
      ...
      add65Tracts(df = tract_merge[[1]], group  = groups[2], fc = ~pal(X65_and_over)) %>%     #2012 Tract Data
      
      # add65Counties(df = county_merge[[2]], group = groups[3], fc = ~pal(X65_and_over)) %>%   #2013 County Data
      add65Tracts(df = tract_merge[[2]], group  = groups[4], fc = ~pal(X65_and_over)) %>%     #2013 Tract Data
      
      # add65Counties(df = county_merge[[3]], group = groups[5], fc = ~pal(X65_and_over)) %>%   #2014 County Data
      add65Tracts(df = tract_merge[[3]], group  = groups[6], fc = ~pal(X65_and_over)) %>%     #2014 Tract Data
      
      # add65Counties(df = county_merge[[4]], group = groups[7], fc = ~pal(X65_and_over)) %>%   #2015 County Data
      add65Tracts(df = tract_merge[[4]], group  = groups[8], fc = ~pal(X65_and_over)) %>%     #2015 Tract Data
      
      # add65Counties(df = county_merge[[5]], group = groups[9], fc = ~pal(X65_and_over)) %>%   #2016 County Data
      add65Tracts(df = tract_merge[[5]], group  = groups[10], fc = ~pal(X65_and_over)) %>%    #2016 Tract Data
      
      # add65Counties(df = county_merge[[6]], group = groups[11], fc = ~pal(X65_and_over)) %>%  #2017 County Data
      add65Tracts(df = tract_merge[[6]], group  = groups[12], fc = ~pal(X65_and_over)) %>%    #2017 Tract Data
      
      # add65Counties(df = county_merge[[7]], group = groups[13], fc = ~pal(X65_and_over)) %>%  #2018 County Data
      add65Tracts(df = tract_merge[[7]], group  = groups[14], fc = ~pal(X65_and_over)) %>%    #2018 Tract Data
